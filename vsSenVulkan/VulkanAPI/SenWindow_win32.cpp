...
{
	SenWindow * window = reinterpret_cast<SenWindow*>(
		GetWindowLongPtrW( hWnd, GWLP_USERDATA ) );

	switch( uMsg ) {
	case WM_CLOSE:
		window->closeSenWindow();
		return 0;
	case WM_SIZE:
		// We get here if the window has changed size, we should rebuild most of our window resources before rendering to this window again.
		break;
	default:
		break;
	}
...
uint64_t	SenWindow::_win32_class_id_counter		= 0;

void SenWindow::_InitOSWindow()
{
	WNDCLASSEX win_class {};
	assert( _surfaceSize_X > 0 );
	assert( _surfaceSize_Y > 0 );
	...
	RECT wr = { 0, 0, LONG( _surfaceSize_X ), LONG( _surfaceSize_Y ) };
